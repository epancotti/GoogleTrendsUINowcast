...
UI_Claims_April18_True <- read_excel("data/StateUIClaims_04182020.xlsx", sheet = "WE0418") %>%
  left_join(tibble(STATE = state.name, location = state.abb)) %>%
  select(location, advance_0418=Advance, updated_0411=`Prior Wk`) %>%
  mutate(location = replace_na(location, "DC"))

UI_Claims_True = UI_Claims_March28_True %>% 
  left_join(UI_Claims_April04_True) %>% 
  left_join(UI_Claims_April11_True) %>% 
  left_join(UI_Claims_April18_True) %>%
  select(-`2 week total`)
